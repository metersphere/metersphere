<template>
  <el-dialog :close-on-click-modal="false" :title="$t('project.code_segment.code_segment')" :visible.sync="visible" :destroy-on-close="true"
             @close="close" width="60%" top="10vh" v-loading="loading" append-to-body class="customFunc">
    <div>
      <el-alert
...
import {getCodeSnippetById, getCodeSnippetPages} from "../../../api/environment";
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      this.loading = getCodeSnippetById(this.currentRow.id).then(res => {
        if (!res.data) {
