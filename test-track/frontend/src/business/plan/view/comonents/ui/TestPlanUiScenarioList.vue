...
import {ENV_TYPE, TEST_PLAN_UI_SCENARIO_CASE} from "metersphere-frontend/src/utils/constants";
  ...
  methods: {
    ...
    search() {
      ...
      if (this.planId) {
        ...
        testPlanUiScenarioList({pageNum: this.currentPage, pageSize: this.pageSize}, this.condition)
          .then(response => {
    ...
    handleRunBatch(config) {
      ...
      if (this.planId) {
        ...
        testPlanUiScenarioCaseRun(param)
          .then(() => {
    ...
    async validateSeleniumSetting() {
      ...
      let validate = true;
      await uiAutomationVerifySeleniumServer()
        .then((res) => {
          if (res.data === "ok") {
            validate = true;
    ...
    remove(row) {
      if (this.planId) {
        testPlanUiScenarioCaseDelete(row.id)
          .then(() => {
        ...
        callback: (action) => {
          if (action === 'confirm') {
            ...
            if (this.planId) {
              ...
              testPlanUiScenarioCaseBatchDelete(param)
                .then(() => {
    ...
    handleBatchEdit() {
      if (this.condition != null && this.condition.selectAll) {
        ...
        testPlanUiScenarioCaseSelectAllTableRows(selectAllRowParams)
          .then(response => {
    ...
    batchEdit(form) {
      ...
      if (this.planId) {
        testPlanUiScenarioCaseBatchUpdateEnv(param)
          .then(() => {
            this.$success(this.$t('commons.save_success'));
            this.search();
          });
