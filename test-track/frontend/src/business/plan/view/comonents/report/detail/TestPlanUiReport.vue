    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane style="min-height: 500px" name="third" v-if="unExecuteEnable">
        ...
        <ui-scenario-result :is-db="isDb" :share-id="shareId" :is-share="isShare"
                            :report="report" :is-template="isTemplate" :plan-id="planId"
                            :ui-all-cases="uiAllCases"
                            :filter-status="['unexecute', 'STOP', 'PENDING', 'STOPPED']"
...
import UiResult from "@/business/plan/view/comonents/report/detail/component/UiResult";
...
import {getPlanUiScenarioAllCase, getSharePlanUiScenarioAllCase,} from "@/api/remote/plan/test-plan";
import UiScenarioResult from "@/business/plan/view/comonents/report/detail/component/UiScenarioResult";
  ...
  methods: {
    ...
    getAllUiCase() {
      if (this.isTemplate || this.isDb) {
        ...
        if (param) {
          getSharePlanUiScenarioAllCase(this.shareId, this.planId, param)
            .then((r) => {
              this.uiAllCases = r.data;
            });
        }
      } else {
        let param = this.getStatusList();
        if (param) {
          this.loading = true; getPlanUiScenarioAllCase(this.planId, param)
            .then((r) => {
              this.uiAllCases = r.data;
            });
        }
