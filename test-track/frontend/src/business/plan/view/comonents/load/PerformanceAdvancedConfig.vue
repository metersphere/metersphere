      ...
      <el-col :span="24">
        ...
        >
          ...
          <el-table-column
            align="center"
            prop="csvHasHeader"
            :label="$t('load_test.csv_has_header')"
          >
            <template v-slot:default="{ row }">
              <el-switch
                :disabled="readOnly || !row.csvSplit"
                v-model="row.csvHasHeader"
              />
            </template>
          </el-table-column>

          <el-table-column
            :label="$t('load_test.csv_file_end_recycle')"
            align="center"
            prop="recycle"
          >
            <template v-slot:default="{ row }">
          ...
          <el-table-column
            :label="$t('load_test.csv_file_end_stop_thread')"
            align="center"
            prop="stopThread"
          >
            <template v-slot:default="{ row }">
          ...
          <el-table-column
            :label="$t('load_test.thread_share')"
            align="center"
            prop="shareMode"
          >
            <template v-slot:default="{ row }">
              <el-select v-model="row.shareMode" :disabled="isReadOnly">
                <el-option
                  key="shareMode.all"
                  :label="$t('load_test.csv_file_end_stop_thread_option.all')"
                  value="shareMode.all"
                ></el-option>
                <el-option
                  key="shareMode.group"
                  :label="$t('load_test.csv_file_end_stop_thread_option.group')"
                  value="shareMode.group"
                ></el-option>
                <el-option
                  key="shareMode.thread"
                  :label="
                    $t('load_test.csv_file_end_stop_thread_option.thread')
                  "
                  value="shareMode.thread"
                ></el-option>
              </el-select>
            </template>
          </el-table-column>
...
import EditMonitor from "@/business/plan/view/comonents/load/EditMonitor";
import BatchAddMonitor from "@/business/plan/view/comonents/load/BatchAddMonitor";
import { testPlanLoadCaseGetAdvancedConfig } from "@/api/remote/plan/test-plan-load-case";
...
export default {
  ...
  methods: {
    getAdvancedConfig() {
      testPlanLoadCaseGetAdvancedConfig(this.loadCaseId).then((response) => {
        if (response.data) {
