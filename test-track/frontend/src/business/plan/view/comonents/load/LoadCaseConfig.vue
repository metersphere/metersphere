<template>
  <el-dialog :close-on-click-modal="false" :visible.sync="visible"
             width="85%" top="8vh" @close="close" v-loading="loading">
...
import PerformanceLoadConfig from "@/business/plan/view/comonents/load/PerformanceLoadConfig";
import PerformanceAdvancedConfig from "@/business/plan/view/comonents/load/PerformanceAdvancedConfig";
import {
  testPlanLoadCaseGet,
  testPlanLoadCaseUpdate
...
export default {
  ...
  methods: {
    open(loadTestId, loadCaseId) {
      ...
      this.loading = true;
      testPlanLoadCaseGet(loadCaseId)
        .then(res => {
          this.loading = false;
