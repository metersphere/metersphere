  ...
  <div v-loading="isReloadData">
    ...
    <el-row>
      <el-col :span="12">
        ...
        <span style="margin-left: 10px">{{ oldData.userName }}</span><span style="margin-left: 10px">{{ oldData.createTime | datetimeFormat }}</span>
      </el-col>
      <el-col :span="12">
        <el-tag>{{ newData.versionName }}</el-tag>
        <span style="margin-left: 10px">{{ newData.userName }}</span><span style="margin-left: 10px">{{ newData.createTime | datetimeFormat }}</span>
      </el-col>
    </el-row>
    <div class="compare-class" id="vdiff" ref="all">
      ...
      <el-card style="width: 50%;" ref="new" id="new">
        ...
        <ms-form-divider :title="$t('load_test.pressure_config')"/>
        <el-row>
          <performance-pressure-config :is-read-only="true" :test="newData" :test-id="newData.id" @fileChange="fileNewChange"
                                       ref="newPressureConfig"/>
        </el-row>
        <ms-form-divider :title="$t('load_test.advanced_config')"/>
        <el-row>
...
import EditPerformanceTest from "@/business/test/EditPerformanceTest";
...
import {diff} from "@/business/v_node_diff";
