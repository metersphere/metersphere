...
public class IssueTemplateService extends TemplateBaseService {
    ...
    public IssueTemplateCopyDTO getCopyProject(String userId, String workspaceId) {
        ...
        while (iterator.hasNext()) {
            ...
            example.createCriteria().andSourceIdEqualTo(projectDto.getId())
                    .andUserIdEqualTo(SessionUtils.getUserId()).andGroupIdIn(Arrays.asList("project_admin", "project_member"));
            ...
            for (GroupPermissionDTO groupPermission : groupPermissions) {
                ...
                boolean issueTemplateChecked = projectTemplatePermissions.getPermissions().stream()
                        .filter(permission -> permission.getChecked() && StringUtils.equals(PermissionConstants.PROJECT_TEMPLATE_READ_ISSUE_TEMPLATE, permission.getId()))
                        .count() == 1L;
                ...
                boolean customChecked = projectTemplatePermissions.getPermissions().stream()
                        .filter(permission -> permission.getChecked() && StringUtils.equals(PermissionConstants.PROJECT_TEMPLATE_READ_CUSTOM, permission.getId()))
                        .count() == 1L;
                if (!customChecked) {
                    // 模板设置中的自定义字段权限未勾选
                    continue;
    ...
    public void copy(CopyIssueTemplateRequest request) {
        ...
        request.getTargetProjectIds().forEach(targetProjectId -> {
            IssueTemplate record = new IssueTemplate();
            ...
            BeanUtils.copyBean(record, sourceIssueTemplate);
            record.setId(UUID.randomUUID().toString());
            record.setName(isExistTargetIssueTemplate ? sourceIssueTemplate.getName() + "_copy" : sourceIssueTemplate.getName());
            record.setCreateTime(System.currentTimeMillis());
            record.setUpdateTime(System.currentTimeMillis());
            record.setCreateUser(SessionUtils.getUserId());
            record.setProjectId(targetProjectId);
            issueTemplateRecords.add(record);
            // 根据复制模式设置自定义字段
            sourceCustomFieldTemplates.forEach(sourceCustomFieldTemplate -> {
                ...
                if (CollectionUtils.isEmpty(targetCustomFields)) {
                    ...
                    tarCustomField.setUpdateTime(System.currentTimeMillis());
                    tarCustomField.setCreateUser(SessionUtils.getUserId());
                    if ("1".equals(request.getCopyModel())) {
                        // 覆盖模式
                        ...
                        if (sourceCustomField.getSystem()) {
                            // 系统字段
                            tarCustomField.setOptions(StringUtils.equals("select", sourceCustomField.getType()) ?
                                    StringUtils.replace(tarCustomField.getOptions(), "]", ",") + StringUtils.replace(sourceCustomField.getOptions(), "[", ""):
                                    "[]");
                            customFieldMapper.updateByPrimaryKeyWithBLOBs(tarCustomField);
                        } else {
                            // 非系统字段, 则追加_copy
                            tarCustomField.setId(UUID.randomUUID().toString());
                            tarCustomField.setName(tarCustomField.getName() + "_copy");
                            tarCustomField.setOptions(sourceCustomField.getOptions());
                            customFieldRecords.add(tarCustomField);
                        }
                    }
                ...
                tarCustomFieldTemplate.setTemplateId(record.getId());
                String defaultValue = sourceCustomFieldTemplate.getDefaultValue() != null && tarCustomField.getOptions().contains(sourceCustomFieldTemplate.getDefaultValue()) ?
                        sourceCustomFieldTemplate.getDefaultValue() : null;
