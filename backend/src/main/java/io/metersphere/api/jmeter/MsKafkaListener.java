...
public class MsKafkaListener {
    ...
    public void debugConsume(ConsumerRecord<?, String> consumerRecord) {
        try {
            LoggerUtil.info("接收到执行结果：", consumerRecord.key());
            if (ObjectUtils.isNotEmpty(consumerRecord.value())
                    && WebSocketUtils.has(consumerRecord.key().toString())) {
                MsgDto dto = JSON.parseObject(consumerRecord.value(), MsgDto.class);
