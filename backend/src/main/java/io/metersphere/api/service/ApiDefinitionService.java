...
public class ApiDefinitionService {
    ...
    private ApiImportSendNoticeDTO importCreate(ApiDefinitionWithBLOBs apiDefinition, ApiDefinitionMapper batchMapper,
        ...
        if (StringUtils.equals("fullCoverage", apiTestImportRequest.getModeId())) {
            return _importCreate(collect, batchMapper, apiDefinition, extApiDefinitionMapper, apiTestImportRequest, mocks, apiTestCaseMapper, caseList);
    ...
    private ApiDefinitionResult getApiDefinitionResult(ApiDefinitionWithBLOBs apiDefinition, boolean isUpdate) {
        ...
        apiDefinitionResult.setUpdate(isUpdate);
    ...
    private List<ApiTestCaseDTO> importCase(ApiDefinitionWithBLOBs apiDefinition, ApiTestCaseMapper apiTestCaseMapper, List<ApiTestCaseWithBLOBs> caseList) {
        ...
        for (int i = 0; i < caseList.size(); i++) {
            ...
            if (StringUtils.isNotBlank(apiTestCaseWithBLOBs.getId())) {
                ...
                BeanUtils.copyBean(apiTestCaseDTO, apiTestCaseWithBLOBs);
                apiTestCaseDTO.setUpdate(false);
    ...
    private ApiImportSendNoticeDTO _importCreate(List<ApiDefinitionWithBLOBs> sameRequest, ApiDefinitionMapper batchMapper, ApiDefinitionWithBLOBs apiDefinition,
                                                 ExtApiDefinitionMapper extApiDefinitionMapper,
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(apiImportSendNoticeDTOS)) {
                for (ApiImportSendNoticeDTO apiImportSendNoticeDTO : apiImportSendNoticeDTOS) {
                    if (apiImportSendNoticeDTO.getApiDefinitionResult() != null && apiImportSendNoticeDTO.getApiDefinitionResult().isUpdate()) {
    ...
    private List<ApiImportSendNoticeDTO> importApi(ApiTestImportRequest request, ApiDefinitionImport apiImport) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            if (apiImport.getEsbApiParamsMap() != null) {
                ...
                ApiImportSendNoticeDTO apiImportSendNoticeDTO = importCreate(item, batchMapper, extApiDefinitionMapper, request, apiImport.getMocks(), updateList, apiTestCaseMapper, caseList);
                if (model != null) {
