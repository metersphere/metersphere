...
public class MetricQuery {
    ...
    public static Map<String, String> getMetricQueryMap() {
        return new HashMap<String, String>(16) {{
            ...
            put("netIn", "sum by (instance) (irate(node_network_receive_bytes_total{instance='%1$s',device!~'bond.*?|lo'}[1m]))");
            put("netOut", "sum by (instance) (irate(node_network_transmit_bytes_total{instance='%1$s',device!~'bond.*?|lo'}[1m]))");
        }};
