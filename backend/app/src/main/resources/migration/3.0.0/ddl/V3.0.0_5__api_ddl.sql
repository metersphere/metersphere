...
CREATE INDEX uk_definition_id_user_id ON api_definition_follower (api_definition_id, user_id);
...
(
    ...
    `report_status`          VARCHAR(50) COMMENT '' 最后一次执行的结果状态 '',
    `num`                    INT COMMENT '' 编号 '',
    ...
    `report_id`              VARCHAR(50) COMMENT '' 最后一次执行的报告fk '',
...
CREATE INDEX idx_report_status ON api_scenario (report_status);
...
CREATE INDEX uk_api_scenario_id_user_id ON api_scenario_follower (api_scenario_id, user_id);
...
(
    ...
    `api_report_status` VARCHAR(20) COMMENT '' 最新执行结果状态 '',
    `api_report_id`     VARCHAR(50) COMMENT '' 最后执行结果报告fk '',
    `pos`               BIGINT       NOT NULL COMMENT '' 自定义排序 '',
    `sync_enable`       BIT(1)       NOT NULL DEFAULT 0 COMMENT '' 是否开启同步 '',
    `sync_time`         BIGINT COMMENT '' 需要同步的开始时间 '',
    `project_id`        VARCHAR(50)  NOT NULL COMMENT '' 项目fk '',
    `api_definition_id` VARCHAR(50)  NOT NULL COMMENT '' 接口fk '',
    `version_id`        VARCHAR(50) COMMENT '' 版本fk '',
...
CREATE INDEX uk_case_id_user_id ON api_test_case_follower (case_id, user_id);
