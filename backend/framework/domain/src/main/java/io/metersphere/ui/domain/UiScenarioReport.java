...
public class UiScenarioReport implements Serializable {
    ...
    @Schema(description =  "触发模式（手动，定时，批量，测试计划）", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description =  "报告类型（集合，独立）")
    private Boolean integrated;

    @Schema(description =  "关联的测试计划报告ID（可以为空)")
    private String relevanceTestPlanReportId;

    @Schema(description =  "报告生成来源(生成报告，后端调试，本地调试)", requiredMode = Schema.RequiredMode.REQUIRED)
