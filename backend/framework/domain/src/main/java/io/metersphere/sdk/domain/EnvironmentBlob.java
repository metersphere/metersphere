...
@Data
public class EnvironmentBlob implements Serializable {
    @Schema(description = "环境ID", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotBlank(message = "{environment_blob.id.not_blank}", groups = {Updated.class})
    @Size(min = 1, max = 50, message = "{environment_blob.id.length_range}", groups = {Created.class, Updated.class})
    private String id;
