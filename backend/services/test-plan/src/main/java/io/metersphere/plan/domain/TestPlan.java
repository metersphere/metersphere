...
public class TestPlan implements Serializable {
    ...
    @Size(min = 1, max = 50, message = "{test_plan.workspace_id.length_range}", groups = {Created.class, Updated.class})
    @NotBlank(message = "{test_plan.workspace_id.not_blank}", groups = {Created.class})
    ...
    @Size(min = 1, max = 128, message = "{test_plan.name.length_range}", groups = {Created.class, Updated.class})
    @NotBlank(message = "{test_plan.name.not_blank}", groups = {Created.class})
    @ApiModelProperty(name = "测试计划名称", required = true, allowableValues="range[1, 128]")
    private String name;
    ...
    @ApiModelProperty(name = "标签", allowableValues="range[1, 1000]")
    private String tags;
    ...
    @ApiModelProperty(name = "描述", allowableValues="range[1, 500]")
