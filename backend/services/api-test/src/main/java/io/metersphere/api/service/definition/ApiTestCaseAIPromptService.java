...
@Service
public class ApiTestCaseAIPromptService {
     ...
     * @param userId 用户ID
     */
    public ApiCaseAIPromptDTO getUserPrompt(String userId) {
        ...
        return JSON.parseObject(configStr, ApiCaseAIPromptDTO.class);
    }
     ...
     */
    public void saveUserPrompt(String userId, ApiCaseAIPromptDTO promptDTO) {
        AiUserPromptConfigExample example = new AiUserPromptConfigExample();
        example.createCriteria().andUserIdEqualTo(userId).andTypeEqualTo(AIConfigConstants.AiPromptType.API_CASE.toString());
