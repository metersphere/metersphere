...
public abstract class PostmanAbstractParserParserApiDefinition<T> extends HttpApiDefinitionImportAbstractParser<T> {
    ...
    protected Map<ApiDefinitionImportDetail, List<ApiDefinitionImportDetail>> parsePostman(PostmanItem requestItem, String modulePath, ImportRequest importRequest) {
        ...
        List<ApiDefinitionImportDetail> postmanExamples = this.parsePostmanRequest(requestItem, modulePath, importRequest);
        return new HashMap<>() {{
    ...
    private List<ApiDefinitionImportDetail> parsePostmanRequest(PostmanItem requestItem, String modulePath, ImportRequest importRequest) {
        List<ApiDefinitionImportDetail> postmanExamples = new ArrayList<>();
        ...
        if (CollectionUtils.isNotEmpty(response)) {
            response.forEach(r -> {
                ...
                ApiDefinitionImportDetail detail = buildApiDefinition(name, url, postmanRequest.getMethod(), modulePath, importRequest);
