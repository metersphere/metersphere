...
public class ApiScenarioService {
    ...
    private void processApiScenario(List<ApiScenarioDTO> scenarioLists) {
        ...
        scenarioLists.forEach(item -> {
            ...
            if (!item.getGrouped() && MapUtils.isNotEmpty(envMap) && envMap.containsKey(item.getEnvironmentId())) {
                item.setEnvironmentName(envMap.get(item.getEnvironmentId()));
            } else if (item.getGrouped() && MapUtils.isNotEmpty(groupMap) && groupMap.containsKey(item.getEnvironmentId())) {
