...
public class ApiScenarioService extends MoveNodeService {
    ...
    public List<String> selectApiIdInCustomRequest(String projectId, List<ApiDefinition> apiDefinitions) {
        ...
        stepConfigList.forEach(step -> {
            if (StringUtils.isNotBlank(step.getConfig())) {
                try {
                    Map<String, Object> stepConfig = JSON.parseMap(step.getConfig());
                    if (StringUtils.equalsIgnoreCase("http", stepConfig.get("protocol").toString())) {
                        requestIdList.add(step.getId());
                    }
                } catch (Exception ignore) {
                }
            }
