...
public class HarParserApiDefinition extends HttpApiDefinitionImportAbstractParser<ApiImportFileParseResult> {
    ...
    public ApiImportDataAnalysisResult generateInsertAndUpdateData(ApiImportFileParseResult importParser, List<ApiDefinitionImportDetail> existenceApiDefinitionList) {
        ...
        // har文件导入要对已有数据进行har文件确实字段的填补。
        this.mergeExistenceApiMap(insertAndUpdateData.getExistenceApiMap());
        return insertAndUpdateData;
    }

    private void mergeExistenceApiMap(Map<ApiDefinitionImportDetail, ApiDefinitionImportDetail> existenceApiMap) {
        for (Map.Entry<ApiDefinitionImportDetail, ApiDefinitionImportDetail> entry : existenceApiMap.entrySet()) {
            ...
            ApiDefinitionImportDetail savedApi = entry.getValue();
            ...
            importHttpElement.setMockNum(existenceHttpElement.getMockNum());
                        ...
                        importHttpElement.setHeaders(this.mergeHeaders(importHttpElement.getBody(),existenceHttpElement.getBody()));
    ...
    private List<ApiDefinitionImportDetail> parseRequests(Har har, ImportRequest importRequest) {
        List<ApiDefinitionImportDetail> resultList = new ArrayList<>();
        ...
        for (HarEntry entry : harEntryList) {
            ...
            if (harRequest != null) {
                ...
                ApiDefinitionImportDetail detail = buildApiDefinition(reqName, url, harRequest.method, null, importRequest);
