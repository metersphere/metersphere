...
import io.metersphere.api.dto.converter.ApiDefinitionPreImportAnalysisResult;
...
public class ApiDefinitionImportService {
    ...
    public void apiDefinitionImport(MultipartFile file, ImportRequest request, String projectId) {
        ...
        try {
            ...
            List<ApiDefinitionImportDetail> existenceApiDefinitionList = extApiDefinitionMapper.importList(pageRequest);
    ...
    private void insertApiDefinition(ImportRequest request, List<ApiDefinitionImportDetail> insertApiData, ApiDefinitionMapper apiMapper, ApiDefinitionBlobMapper apiBlobMapper, SqlSession sqlSession) {
    ...
    private void insertApiTestCase(ImportRequest request, List<ApiTestCaseDTO> insertApiCaseData, ApiTestCaseMapper apiTestCaseMapper, ApiTestCaseBlobMapper apiTestCaseBlobMapper, SqlSession sqlSession) {
        insertApiCaseData.forEach(t -> {
            ...
            apiTestCaseBlob.setRequest(JSON.toJSONBytes(t.getRequest()));
            apiTestCaseBlobMapper.insertSelective(apiTestCaseBlob);
        });
        sqlSession.flushStatements();
    }

    private static void updateApiDefinition(ImportRequest request, List<ApiDefinitionImportDetail> updateRequestData, ApiDefinitionMapper apiMapper, ApiDefinitionBlobMapper apiBlobMapper, SqlSession sqlSession) {
    ...
    private static void updateApiDefinitionModule(ImportRequest request, List<ApiDefinitionImportDetail> updateModuleData, ApiDefinitionMapper apiMapper, SqlSession sqlSession) {
    ...
    private void inertDataAnalysis(ApiDefinitionPreImportAnalysisResult apiDefinitionPreImportAnalysisResult, ImportRequest request, Map<String, BaseTreeNode> modulePathMap, ApiImportDataAnalysisResult analysisResult) {
        for (ApiDefinitionImportDetail apiData : analysisResult.getInsertApiList()) {
    ...
    private void existenceDataAnalysis(ApiDefinitionPreImportAnalysisResult apiDefinitionPreImportAnalysisResult, ImportRequest request, Map<String, BaseTreeNode> modulePathMap, ApiImportDataAnalysisResult analysisResult) {
        //不选择覆盖接口或者数据为空：终止操作
        if (MapUtils.isEmpty(analysisResult.getExistenceApiMap()) || !request.isCoverData()) {
            return;
        }
        Map<ApiDefinitionImportDetail, ApiDefinitionImportDetail> existenceApiMap = analysisResult.getExistenceApiMap();
        List<String> existenceApiIdList = existenceApiMap.values().stream().map(ApiDefinitionImportDetail::getId).toList();
        ...
        for (Map.Entry<ApiDefinitionImportDetail, ApiDefinitionImportDetail> entry : existenceApiMap.entrySet()) {
            ApiDefinitionImportDetail importApi = entry.getKey();
            ApiDefinitionImportDetail existenceApi = entry.getValue();
     ...
     */
    private void updateApiDefinitionModule(ApiDefinitionImportDetail importApi, Map<String, BaseTreeNode> modulePathMap, String importModuleId, ApiDefinitionPreImportAnalysisResult apiDefinitionPreImportAnalysisResult) {
    ...
    private void updateApiDefinitionRequest(ApiDefinitionImportDetail importApi, MsHTTPElement existenceMsHTTPElement, String platform) {
