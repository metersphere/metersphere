...
public class FunctionalCaseFileService {
     ...
     * @return
     */
    public FunctionalCaseImportResponse preCheckExcel(FunctionalCaseImportRequest request, String userId, MultipartFile file) {
        if (file == null) {
            throw new MSPluginException("file_cannot_be_null");
        }
        FunctionalCaseImportResponse response = new FunctionalCaseImportResponse();
        checkImportExcel(response, request, userId, file);
        return response;
    }

    private void checkImportExcel(FunctionalCaseImportResponse response, FunctionalCaseImportRequest request, String userId, MultipartFile file) {
        try {
            ...
            FunctionalCaseImportEventListener eventListener = new FunctionalCaseImportEventListener(request, clazz, customFields, mergeInfoSet);
