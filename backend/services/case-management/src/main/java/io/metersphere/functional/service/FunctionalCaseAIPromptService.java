...
@Service
public class FunctionalCaseAIPromptService {
     ...
     */
    public FunctionalCaseAIPromptDTO getUserPrompt(String userId) {
        ...
        if (CollectionUtils.isEmpty(aiUserPromptConfigs)) {
            return new FunctionalCaseAIPromptDTO();
        }
        ...
        return JSON.parseObject(configStr, FunctionalCaseAIPromptDTO.class);
    }
     ...
     */
    public void saveUserPrompt(String userId, FunctionalCaseAIPromptDTO promptDTO) {
        AiUserPromptConfigExample example = new AiUserPromptConfigExample();
        example.createCriteria().andUserIdEqualTo(userId).andTypeEqualTo(AIConfigConstants.AiPromptType.FUNCTIONAL_CASE.toString());
