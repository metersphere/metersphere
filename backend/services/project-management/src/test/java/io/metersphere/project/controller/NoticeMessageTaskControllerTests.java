...
public class NoticeMessageTaskControllerTests extends BaseTest {
    ...
    void addMessageTask() throws Exception {
        ...
        /*MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/notice/message/task/data/project-message-test")
                        .header(SessionConstants.HEADER_TOKEN, sessionId)
                        .header(SessionConstants.CSRF_TOKEN, csrfToken))
                .andExpect(status().isOk()).andReturn();
        String contentAsString = mvcResult.getResponse().getContentAsString(StandardCharsets.UTF_8);;
        ResultHolder resultHolder = JSON.parseObject(contentAsString, ResultHolder.class);
        System.out.println(JSON.toJSONString(resultHolder.getData()));*/
    }
