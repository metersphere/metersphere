...
public class CustomFieldUtils {
     ...
     * @param request 请求参数
     */
    public static void setBaseQueryRequestCustomMultipleFields(BaseCondition request) {
        handleFilterCustomMultipleFields(request);
        handleCombineFields(request);
    }
    ...
    private static void handleCombineFields(BaseCondition request) {
        ...
        if (MapUtils.isNotEmpty(combine)) {
            combine.forEach((k, v) -> {
                if (StringUtils.equals(k, COMBINE_CUSTOM) && ObjectUtils.isNotEmpty(v)) {
                    handleCombineCustomFields((List<Map<String, Object>>) v);
                } else {
                    handleCombineField(v);
                }
            });
        }
    }

    private static void handleCombineCustomFields(List<Map<String, Object>> customs) {
    ...
    private static void handleCombineField(Object v) {
