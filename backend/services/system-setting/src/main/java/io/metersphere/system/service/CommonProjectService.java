...
public class CommonProjectService {
    ...
    public void checkOrgRoleExit(String userId, String orgId, String createUser, String userName, String path, String module) {
        ...
        setLog(orgId, orgId, path,  module,Translator.get("add") + Translator.get("organization_member") + ": " + userName, createUser, OperationLogType.ADD.name(), HttpMethodConstants.POST.name(), logDTOList);
        ...
        if (CollectionUtils.isNotEmpty(updateProjectDto.getUserIds())) {
            ...
            if (CollectionUtils.isNotEmpty(deleteIds)) {
                ...
                deleteIds.forEach(userId -> {
                    ...
                    setLog(project.getId(), project.getOrganizationId(), path, module,Translator.get("delete") + Translator.get("project_admin") + ": " + user.getName(), updateUser, OperationLogType.DELETE.name(), HttpMethodConstants.POST.name(),logDTOList);
                });
            }
        ...
        request.getProjectIds().forEach(projectId -> {
            ...
            request.getUserIds().forEach(userId -> {
                ...
                if (userRoleRelationMapper.selectByExample(userRoleRelationExample).size() == 0) {
                    ...
                    setLog(projectId, project.getOrganizationId(), path, module,content + Translator.get("project_admin") + ": " + user.getName(), createUser, type, method, logDTOList);
                }
            });
        });
        ...
        request.getProjectIds().forEach(projectId -> {
            ...
            request.getUserIds().forEach(userId -> {
                ...
                if (userRoleRelationMapper.selectByExample(userRoleRelationExample).size() == 0) {
                    ...
                    setLog(projectId,project.getOrganizationId(), path, module,content + Translator.get("project_member") + ": " + user.getName(), createUser, type, method, logDTOList);
                }
            });
        });
        userRoleRelationMapper.batchInsert(userRoleRelations);
    ...
    public int removeProjectMember(String projectId, String userId,String createUser, String module, String path) {
        ...
        List<LogDTO> logDTOList = new ArrayList<>();
        setLog(projectId, StringUtils.EMPTY,path,
                module,Translator.get("delete") + Translator.get("project_member") + ": " + user.getName(),
                createUser, OperationLogType.DELETE.name(), HttpMethodConstants.GET.name(), logDTOList);
        operationLogService.batchAdd(logDTOList);
    ...
    public void deleteProject(List<Project> projects) {
        ...
        projects.forEach(project -> {
            ...
            setLog(project.getId(), project.getOrganizationId(), StringUtils.EMPTY,OperationLogModule.SYSTEM_PROJECT,Translator.get("delete") + Translator.get("project") + ": " + project.getName(),
     ...
     */
    private void setLog(String projectId, String orgId, String path, String module,
                               String content, String userId,
                               String type, String method, List<LogDTO> logDTOList) {
        LogDTO dto = new LogDTO(
                StringUtils.equals(OperationLogModule.SYSTEM_PROJECT, module) ? OperationLogConstants.SYSTEM : OperationLogConstants.ORGANIZATION,
                StringUtils.equals(OperationLogModule.SYSTEM_PROJECT, module) ? OperationLogConstants.SYSTEM : orgId,
                projectId,
                userId,
                type,
                module,
