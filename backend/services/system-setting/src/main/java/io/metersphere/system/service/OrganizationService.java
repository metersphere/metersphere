...
public class OrganizationService{
    ...
    public void addMemberToProject(OrgMemberExtendProjectRequest orgMemberExtendProjectRequest, String userId) {
        ...
        orgMemberExtendProjectRequest.getMemberIds().forEach(memberId->{
            orgMemberExtendProjectRequest.getProjectIds().forEach(projectId->{
                //过滤已存在的关系
                UserRoleRelationExample example = new UserRoleRelationExample();
                ...
                if (CollectionUtils.isEmpty(userRoleRelations)) {
                    ...
                    userRoleRelationMapper.insertSelective(userRoleRelation);
                    //add Log
                    setLog(orgMemberExtendProjectRequest.getOrganizationId(),projectId,"/organization/project/add-member",OperationLogModule.PROJECT_PROJECT_MEMBER,"", logDTOList, memberId, userRoleRelation, true);
                }
            });
        });
        sqlSession.flushStatements();
