...
public class BugControllerTests extends BaseTest {
    ...
    @Test
    @Order(96)
    void coverPlatformTemplate() throws Exception{
        // 覆盖同步缺陷(Local)
    ...
    @Test
    @Order(96)
    void coverPlatformBugSyncTest() throws Exception {
        // 添加Jira缺陷
        ...
        // 删除唯一条平台缺陷
        this.requestGet(BUG_DELETE + "/" + updateRequest2.getId(), status().isOk());
        // 同步Jira存量缺陷(存量数据为空)
        this.requestGetWithOk(BUG_SYNC + "/default-project-for-bug");
