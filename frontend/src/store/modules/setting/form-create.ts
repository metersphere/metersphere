...
import type { FomItem, FormRuleItem } from '@/components/pure/ms-form-create/types';
...
const useFormCreateStore = defineStore('form-create', {
  persist: false,
  state: (): {
    formRuleMap: Map<FormCreateKeyEnum[keyof FormCreateKeyEnum], FomItem[]>;
  ...
  actions: {
    ...
    setInitFormCreate(key: FormCreateKeyEnum[keyof FormCreateKeyEnum], formRule: FomItem[]) {
    ...
    initFormCreateFormRules(key: FormCreateKeyEnum[keyof FormCreateKeyEnum]) {
      ...
      // 处理数据结构
      const result = currentFormRule?.map((item: FomItem) => {
        // 当前类型
        let fieldType;
