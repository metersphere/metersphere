<template>
  ...
  <div>
    ...
    <div class="ms-border">
      <el-table :data="sslConfig.files" highlight-current-row @current-change="selectRow" v-if="!loading">
    ...
    <div class="ms-border">
      <el-table :data="sslConfig.entrys" highlight-current-row @current-change="selectRow" v-if="!loading">
  ...
  export default {
    ...
    methods: {
      ...
      addConfig(config, file) {
        let sslFile = {id: getUUID(), name: file.name, type: file.type, updateTime: new Date().getTime(), password: config.password, file: file};
      ...
      remove(row) {
        ...
        if (this.sslConfig.entrys) {
          for (let i in this.sslConfig.entrys) {
            let entry = this.sslConfig.entrys[i];
            if (entry.sourceId === row.id) {
