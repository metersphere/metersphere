...
export default {
  ...
  created() {
    ...
    if(STEP()) {
      this.operatingElements = STEP().get("ALL");
    }
    this.getWsProjects();
  ...
  methods: {
    ...
    nodeClick(data, node) {
      if (data.referenced != 'REF' && data.referenced != 'Deleted' && !data.disabled && STEP()) {
        this.operatingElements = STEP().get(data.type);
      } else {
        this.operatingElements = [];
      }
      if (!this.operatingElements && STEP()) {
        this.operatingElements = STEP().get("ALL");
      }
    ...
    allowDrop(draggingNode, dropNode, dropType) {
      ...
      if (dropType != "inner") {
        ...
        && (STEP().get(dropNode.data.type) && STEP().get(dropNode.data.type).indexOf(draggingNode.data.type) != -1)
