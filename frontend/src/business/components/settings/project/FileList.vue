  ...
  <el-dialog :close-on-click-modal="false"
    ...
    <ms-table-header :is-tester-permission="true" title="" :condition.sync="condition" @search="getProjectFiles" :show-create="false">
      <template v-slot:button>
        <ms-table-button :is-tester-permission="true" style="margin-bottom: 10px" icon="el-icon-upload2"
            :content="$t('load_test.upload_file')" @click="openFileConfig"/>
        <el-upload
          v-if="loadType === 'jmx'"
...
<!--                             @deleteClick="handleDelete(scope.row)">-->
          <span>
            ...
            <ms-table-operator-button :isTesterPermission="true" icon="el-icon-delete" :tip="$t('commons.delete')" type="danger" @exec="handleDelete(scope.row)"/>
          </span>
<!--          </ms-table-operator>-->
        </template>
      </el-table-column>
  ...
  <el-dialog :visible.sync="fileConfigVisible" :modal-append-to-body="false">
    <files-config-form :callback="sad" :read-only="false" :acceptFileType="acceptFileType" :acceptFileString="acceptFileString"></files-config-form>
  </el-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    name: "FileList",
    components: {MsTableHeader, MsTableButton, MsTablePagination, MsDialogFooter, MsTableOperator, HeaderLabelOperate, MsTableOperatorButton, FilesConfigForm},
    props: {
      fileList: Array,
      tableData: Array,
      uploadList: Array,
    ...
    computed: {
      acceptFileType: function () {
        if(this.loadType === 'jmx') {
          return ['jmx'];
        } else {
          return ['jar', 'csv', 'json', 'pdf', 'jpg', 'png', 'jpeg', 'doc', 'docx', 'xlsx', 'txt'];
        }
      },
      acceptFileString: function () {
        if(this.loadType === 'jmx') {
          return '.jmx';
        } else {
          return '.jar,.csv,.json,.pdf,.jpg,.png,.jpeg,.doc,.docx,.xlsx,.txt';
    ...
    methods: {
      sad(config, file) {
        ...
        this.projectLoadingResult = this.$post('/project/upload/files/' + config.projectId, file, res => {
      ...
      handleDelete(item) {
        this.projectLoadingResult = this.$post('/performance/file/delete/' + item.id, res => {
        });
        //  未刷新
      },
      open(loadType) {
