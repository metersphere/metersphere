...
import type { PluginList, PluginItem, AddReqData, UpdatePluginModel } from '@/models/setting/plugin';
...
export function deletePluginReq(id: string) {
  return MSR.get<PluginItem>({ url: `${DeletePluginUrl}/${id}` });
}
export function getScriptDetail(pluginId: string, scriptId: string) {
  return MSR.get({ url: `${GetScriptUrl}/${pluginId}/${scriptId}` });
}
