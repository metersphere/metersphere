        ...
        <a-form ref="activeApiTabFormRef" :model="scenario" layout="vertical">
          ...
          <a-form-item :label="t('apiScenario.status')" class="mb-[16px]">
            ...
            >
              ...
              <a-option v-for="item of caseStatusOptions" :key="item.value" :value="item.value">
                <apiStatus :status="item.value" />
              </a-option>
            </a-select>
          </a-form-item>
...
<script setup lang="ts">
  ...
  import { RequestCaseStatus, ScenarioCreateComposition } from '@/enums/apiEnum';

  import { caseStatusOptions } from '@/views/api-test/components/config';
  // 组成部分异步导入
