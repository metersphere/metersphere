      ...
      >
        <template #columns>
          ...
          <a-table-column :title="t('system.plugin.tableColunmApplicationScene')" data-index="scenario">
            <template #cell="{ record }">{{
              record.scenario === 'API' ? t('system.plugin.sceneApi') : t('system.plugin.sceneProManger')
            }}</template>
          </a-table-column>
          <a-table-column :title="t('system.plugin.tableColunmOrg')" :width="300">
            <template #cell="{ record }">
...
<script setup lang="ts">
  ...
  const sceneList = ref([
    ...
    {
      label: 'system.plugin.proManagement',
      value: 'PLATFORM',
    },
  ]);
