...
<script setup lang="ts">
  ...
  const showSelectField = () => {
    ...
    // 回显系统字段
    selectSystemData.value = selectSystemData.value.filter((id) => selectIds.indexOf(id) > -1);
    // 回显自定义字段
    selectCustomField.value = selectCustomField.value.filter((id) => selectIds.indexOf(id) > -1);
  };
