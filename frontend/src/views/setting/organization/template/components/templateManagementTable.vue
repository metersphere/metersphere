<template>
  ...
  <AddFieldToTemplateDrawer
    ...
    :selected-data="(data as DefinedFieldItem[])"
...
<script setup lang="ts">
  ...
  import { allFieldTypeFormRules, getIconType } from './fieldSetting';
  ...
  const getFieldOptionList = () => {
    totalTemplateField.value = totalTemplateField.value.map((item) => {
      // 处理选项
      const currentFormRules = allFieldTypeFormRules[item.type];
