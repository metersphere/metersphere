...
<script setup lang="ts">
  ...
  import type { AddOrUpdateField, fieldIconAndNameModal, fieldTypes } from '@/models/setting/template';
  ...
  const selectFormat = ref<fieldTypes>(); // 选择格式
  const isMultipleSelectMember = ref<boolean | undefined>(false); // 成员多选
  const fieldType = ref<fieldTypes>(); // 整体字段类型

  // 是否展示选项添加面板
  const showOptionsSelect = computed(() => {
    const showOptionsType: fieldTypes[] = ['RADIO', 'CHECKBOX', 'SELECT', 'MULTIPLE_SELECT'];
  ...
  // 处理特殊情况编辑回显
  const getSpecialHandler = (itemType: fieldTypes): fieldTypes => {
    switch (itemType) {
      case 'INT':
