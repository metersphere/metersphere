<template>
  <MsCard simple class="wrapper mb-[16px]">
    <div
      class="outer-wrapper"
      :style="{
        height: `calc(100vh - ${listHeight} - 168px)`,
      }"
    >
      ...
      <div class="ms-card-wrap">
        <a-scrollbar
          :style="{
            overflow: 'auto',
            height: `calc(100vh - ${listHeight} - 240px)`,
          }"
        >
          <div class="list">
...
<script setup lang="ts">
  ...
  const collHeight = ref('72px');
  watchEffect(() => {
    collHeight.value = props.listHeight;
  });
...
<style scoped lang="less">
  .wrapper :deep(.ms-card) {
    max-height: calc(100vh - v-bind(collheight) - 120px) !important;
  }
  .ms-card-wrap {
    overflow: hidden;
    padding: 8px;
