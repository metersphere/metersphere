        ...
        <el-table-column :label="$t('commons.name')" prop="name" show-overflow-tooltip min-width="200">
          <template v-slot:default="scope">
            <span @click.stop v-if="scope.row.showNameInput">
              <el-input size="mini" v-model="scope.row.name" class="name-input" @blur="updateGroupName(scope.row)"
                        show-word-limit
...
<script>
import {copyEnvironment, delEnvironment, envGroupList, environmentGroupModify} from "metersphere-frontend/src/api/environment";
