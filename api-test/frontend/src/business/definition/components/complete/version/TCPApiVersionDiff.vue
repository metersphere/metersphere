      ...
      <el-card style="width: 50%;" ref="old">
        <div style="background-color: white;">
          ...
          <div v-else-if="oldApiProtocol==='ESB'">
            <p class="tip">{{ $t('api_test.definition.request.req_param') }} </p>
            <mx-esb-definition v-xpack :show-script="false" :is-read-only="true"
                               :request="oldRequest" ref="esbDefinition"/>
            <p class="tip">{{ $t('api_test.definition.request.res_param') }}</p>
            <mx-esb-definition-response v-xpack :is-api-component="true" :is-read-only="true"
                                        :request="oldRequest"/>
              ...
              <el-collapse-transition>
                <el-tabs v-model="activeName" style="margin: 20px">
                  <el-tab-pane :label="$t('commons.remark')" name="remark" class="pane">
                    <form-rich-text-item class="remark-item" :disabled="true" :data="oldData" prop="remark"
                                         label-width="0"/>
                  </el-tab-pane>
                  <el-tab-pane :label="$t('commons.relationship.name')" name="dependencies" class="pane">
                    ...
                    <dependencies-list @setCount="setOldCount" :read-only="true" :resource-id="oldData.id"
                                       resource-type="API" ref="oldDependencies"/>
                  </el-tab-pane>
                </el-tabs>
              </el-collapse-transition>
            </el-form>
      ...
      <el-card style="width: 50%;" ref="new">
        <div style="background-color: white;">
          ...
          <div v-else-if="apiProtocol=='ESB'">
            <p class="tip">{{ $t('api_test.definition.request.req_param') }} </p>
            <mx-esb-definition v-xpack  :show-script="false" :is-read-only="true"
                               :request="request" ref="esbDefinition"/>
            <p class="tip">{{ $t('api_test.definition.request.res_param') }}</p>
            <mx-esb-definition-response v-xpack :is-api-component="true" :is-read-only="true"
                                        :request="request"/>
          </div>
          ...
          <api-info-container>
            <el-form :model="newData" ref="api-form" label-width="100px">
              <el-collapse-transition>
                <el-tabs v-model="activeName" style="margin: 20px">
                  <el-tab-pane :label="$t('commons.remark')" name="remark" class="pane">
                    <form-rich-text-item class="remark-item" :disabled="true" :data="newData" prop="remark"
                                         label-width="0"/>
                  </el-tab-pane>
                  <el-tab-pane :label="$t('commons.relationship.name')" name="dependencies" class="pane">
                    <template v-slot:label>
                    ...
                    <dependencies-list @setCount="setCount" :read-only="true" :resource-id="newData.id"
                                       resource-type="API" ref="newDependencies"/>
                  </el-tab-pane>
                </el-tabs>
              </el-collapse-transition>
            </el-form>
