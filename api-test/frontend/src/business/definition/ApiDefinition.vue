    ...
    <ms-container v-if="renderComponent">
      <ms-aside-container v-show="isAsideHidden">
        <ms-api-module
          ...
          @schedule="
            handleTabsEdit(
              $t('api_test.api_import.timing_synchronization'),
              'SCHEDULE'
            )
          "
          :type="'edit'"
      ...
      <ms-main-container>
        ...
        >
          ...
          >
            ...
            <!-- 添加/编辑测试窗口-->
            <div
              v-if="item.type === 'ADD' || item.type === 'TEST'"
              class="ms-api-div"
            >
            ...
            <template v-slot:label>
              <el-dropdown @command="handleCommand">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    command="debug"
                    v-permission="['PROJECT_API_DEFINITION:READ+DEBUG']"
                  >
...
import {
  getApiModuleByProjectIdAndProtocol,
  trashCount,
...
export default {
  ...
  watch: {
    ...
    apiDefaultTab() {
      this.isAsideHidden =
        this.apiDefaultTab === "default" || this.apiDefaultTab === "trash";
    },
  },
  created() {
    let routeParamObj = this.$route.params;
  ...
  methods: {
    ...
    openSwaggerScheduleTab() {
      ...
      if (scheduleTabName === "") {
        this.handleTabsEdit(
          this.$t("api_test.api_import.timing_synchronization"),
          "SCHEDULE"
        );
      } else {
    ...
    addEnv(envId) {
      createDefinitionEnv({ userId: getCurrentUserId(), envId: envId }).then(
        (response) => {}
      );
    },
    getTrashApi() {
    ...
    getEnv() {
      getDefinitionEnv(getCurrentUserId(), getCurrentProjectID()).then(
        (response) => {
    ...
    handleTabClose() {
      ...
      tab.forEach((t) => {
        if (
          ...
          t.type === "ADD" &&
          t.title === this.$t("api_test.definition.request.title")
        ) {
    ...
    editApiModule(row) {
      getApiModuleByProjectIdAndProtocol(row.projectId, row.protocol).then(
        (response) => {
          if (response.data) {
    ...
    editApi(row) {
      ...
      if (!index) {
        ...
        if (row.isCopy) {
          ...
          if (row.name) {
            name =
              this.$t("api_test.definition.request.edit_api") + "-" + row.name;
          } else {
