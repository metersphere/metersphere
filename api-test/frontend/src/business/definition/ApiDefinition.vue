<template>
  <div>
          ...
          <el-tab-pane
            ...
            >
              <template v-slot:version>
                <mx-version-select v-xpack :project-id="projectId" :version-id="trashVersion"
    ...
    <mock-edit-drawer :is-tcp=false :api-id="mockConfigData.apiId"
...
export default {
  ...
  methods: {
    ...
    handleTabClose() {
      ...
      tab.forEach(t => {
        if (t.type === 'ADD' && t.api && store.apiMap.has(t.api.id) && (store.apiMap.get(t.api.id).get("responseChange") === true || store.apiMap.get(t.api.id).get("requestChange") === true ||
      ...
      tab.forEach(t => {
        if (t.name === targetName) {
          if (t.api && store.apiMap.size > 0 && store.apiMap.has(t.api.id)) {
            id = t.api.id;
            if (store.apiMap.get(t.api.id).get("responseChange") === true || store.apiMap.get(t.api.id).get("requestChange") === true ||
              store.apiMap.get(t.api.id).get("fromChange") === true) {
              message += t.api.name;
              id = t.api.id;
            }
