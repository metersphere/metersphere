...
public class ApiDefinitionService {
    ...
    public ApiDefinitionImport apiTestImport(MultipartFile file, ApiTestImportRequest request) {
        ...
        try {
            ...
            // 发送通知
            ApiDefinitionImportUtil.sendFailMessage(request, project);
            throw e;
        } catch (Exception e) {
            // 发送通知
            ApiDefinitionImportUtil.sendFailMessage(request, project);
        ...
        try {
            ApiDefinitionImportParamDTO apiDefinitionImportParamDTO = getApiDefinitionImportParamDTO();
            List<ApiImportSendNoticeDTO> apiImportSendNoticeDTOS = ApiDefinitionImportUtil.importApi(request, apiImport,apiDefinitionImportParamDTO);
            ...
            // 发送通知
            ApiDefinitionImportUtil.sendImportNotice(request, apiImportSendNoticeDTOS,project);
        } catch (Exception e) {
            ApiDefinitionImportUtil.sendFailMessage(request, project);
            LogUtil.error(e);
    ...
    @NotNull
    private ApiDefinitionImportParamDTO getApiDefinitionImportParamDTO() {
        ApiDefinitionImportParamDTO apiDefinitionImportParamDTO = new ApiDefinitionImportParamDTO();
        apiDefinitionImportParamDTO.setSqlSessionFactory(sqlSessionFactory);
        apiDefinitionImportParamDTO.setCurrentApiOrder(currentApiOrder);
        apiDefinitionImportParamDTO.setCurrentApiCaseOrder(currentApiCaseOrder);
        apiDefinitionImportParamDTO.setBaseProjectVersionMapper(baseProjectVersionMapper);
        apiDefinitionImportParamDTO.setProjectMapper(projectMapper);
        apiDefinitionImportParamDTO.setExtApiDefinitionMapper(extApiDefinitionMapper);
        apiDefinitionImportParamDTO.setApiModuleMapper(apiModuleMapper);
        apiDefinitionImportParamDTO.setApiTestCaseMapper(apiTestCaseMapper);
        return apiDefinitionImportParamDTO;
    }
